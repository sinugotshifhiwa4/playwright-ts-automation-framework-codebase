...
Two project-specific rules matter here, and both create silent failures this hunt should catch:

- **`console.log` is banned**, by ESLint and by `guard-staged.mjs`. An error surfaced only to stdout is not surfaced. Treat a caught error that reaches no structured logger as swallowed.
- **Playwright specs**: an unconditional `.skip` or `.fixme` is a test that silently stops testing. `playwright/no-skipped-test` allows a _conditional_ skip because it self-corrects; an unconditional one never runs again and nobody notices. `expect-expect` guards the same failure mode — a test that asserts nothing passes vacuously.

Also flag an `await`-less Playwright call: the assertion resolves after the test ends, so it can never fail the run.
