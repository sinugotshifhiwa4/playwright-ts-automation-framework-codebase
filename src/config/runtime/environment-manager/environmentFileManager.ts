...
export default class EnvironmentFileManager {
   ...
   */
  private constructor() {
    // Intentionally empty. The constructor exists only to be private.
  }
   ...
   * @returns A read-only array of the file names that were loaded.
   */
  ...
  /**
   * Loads the environment file for the current stage.
   * @returns A promise that resolves when the stage's environment file has been loaded.
   */
  private async loadAllEnvironments(): Promise<void> {
    ...
    await this.loadEnvironmentFile(stageFilePath, "stage", (fp) =>
      this.logEnvironmentFileNotFound(fp, env),
    );
  }
   ...
   * @returns A promise that resolves to true when the file was loaded, false when it was missing or failed to load.
   */
   ...
   * @param env - The environment stage to resolve a file path for.
   * @returns The path to that stage's environment file.
   * @throws Error if the stage is not one of the known environment stages.
   */
  ...
  /**
   * Logs a warning when the environment file for a stage cannot be found.
   * @param filePath - Path that was not found.
   * @param environmentStage - Stage name used for logging context.
   */
  private logEnvironmentFileNotFound(filePath: string, environmentStage: string): void {
    logger.warn(
